...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const scrollThreshold = 50; // عدد البيكسلات قبل بدء إخفاء الـ header
    ...
    window.addEventListener('scroll', () => {
        ...
        if (currentScroll > scrollThreshold) {
            if (currentScroll > lastScrollTop) {
                // السكرول للأسفل - إخفاء الـ header
                header.classList.add('hide-header');
            } else {
                // السكرول للأعلى - إظهار الـ header
                header.classList.remove('hide-header');
            }
        } else {
            // في الأعلى - دائماً إظهار الـ header
            header.classList.remove('hide-header');
        }
    ...
    const handleTabRouting = () => {
        ...
        if (['#home', '#lessons', '#quiz'].includes(hash)) {
            ...
            if (hash === '#home') {
                ...
                header.classList.remove('hide-quiz-header');
                ...
                // keep task-bar visible when browsing quiz categories (hide only during an active quiz)
                header.classList.remove('hide-quiz-header');
            }
        }
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function closeLessonPage() {
        ...
        // ensure task-bar is visible when closing lesson/quiz views
        if (header) header.classList.remove('hide-quiz-header');
    ...
    if (showQuizBtn) {
        showQuizBtn.addEventListener('click', () => {
            ...
            // hide the task-bar when the user starts the lesson quiz
            if (header) header.classList.add('hide-quiz-header');
    ...
    if (submitQuizBtn) {
        submitQuizBtn.addEventListener('click', () => {
            ...
            // restore the task-bar after finishing the quiz
            if (header) header.classList.remove('hide-quiz-header');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.diff-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            renderGeneralQuiz();
            // hide the task-bar when the general quiz starts
            if (header) header.classList.add('hide-quiz-header');
        });
    });
    ...
    if (submitGeneralQuizBtn) {
        submitGeneralQuizBtn.addEventListener('click', () => {
            ...
            // restore the task-bar after finishing the general quiz
            if (header) header.classList.remove('hide-quiz-header');
        });
    }
    ...
    if (retryGeneralQuizBtn) {
        retryGeneralQuizBtn.addEventListener('click', () => {
            renderGeneralQuiz();
            // re-hide the task-bar while retrying the quiz
            if (header) header.classList.add('hide-quiz-header');
        });
    }
    ...
    if (closeGeneralQuizBtn) {
        closeGeneralQuizBtn.addEventListener('click', () => {
            ...
            quizCategories.style.display = 'grid';
            // ensure task-bar returns when closing the general quiz
            if (header) header.classList.remove('hide-quiz-header');
        });
    }
